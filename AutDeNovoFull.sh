...
then
data="Ill"
elif [[ !(-z "$fwd") && !(-z "$ont") && -z "$pb" ]]
then
data="IllONT"
elif [[ !(-z "$fwd") && !(-z "$pb") && -z "$ont" ]]
then
data="IllPB"
elif [[ !(-z "$fwd") && !(-z "$pb") && !(-z "$ont") ]]
then
...
if [[ !(-z $ont) ]]
then
  cat ${ont}/*fastq.gz > ${out}/data/${name}_ont.fq.gz &
  cp ${ont}/sequencing_summary.txt ${out}/data/${name}_sequencing_summary.txt
fi
...
#
# pigz -c ${out}/results/assembly/${name}/scaffolds.fasta \
# > ${out}/output/${name}_scaffolds.fa.gz
