...
## AutDeNovo -v 0.2.0 - 30/05/2022 - Skip Trimming & Single-End Edition

## Author: Martin Kapun, based on the pipeline of TBG Loewe@Senkenberg/Frankfurt from Tilman Schell
## Extended with skip trimming and single-end support features

###############################################
############### NEW FEATURES ##################
###############################################
##
## This version adds two new features to the original AutDeNovo_exp.sh:
##
## 1. SKIP TRIMMING (SkipTrimming=yes):
##    - Bypasses the trimming step entirely
##    - Copies and renames raw input files to expected trimmed file names
##    - Allows downstream processing to work seamlessly with raw reads
##    - Useful when reads are already high quality or pre-trimmed
##
## 2. SINGLE-END READS (SingleEnd=yes OR only Fwd= specified):
##    - Supports processing of forward-only reads (no reverse reads)
##    - Automatically detected when Rev= is empty or SingleEnd=yes is set
##    - Processes only the forward read file through the pipeline
##    - Compatible with both trimming and skip-trimming modes
##
## USAGE EXAMPLES:
##
## Skip trimming with paired-end reads:
## ./AutDeNovo_exp_skiptrim.sh Name=Sample1 OutputFolder=/output \
##   Fwd=/reads/sample_R1.fq.gz Rev=/reads/sample_R2.fq.gz \
##   SkipTrimming=yes [other parameters...]
##
## Single-end reads with trimming:
## ./AutDeNovo_exp_skiptrim.sh Name=Sample1 OutputFolder=/output \
##   Fwd=/reads/sample.fq.gz SingleEnd=yes [other parameters...]
##
## Single-end reads without trimming:
## ./AutDeNovo_exp_skiptrim.sh Name=Sample1 OutputFolder=/output \
##   Fwd=/reads/sample.fq.gz SkipTrimming=yes SingleEnd=yes [other parameters...]
##
###############################################
...
AutDeNovo v. 0.02 - 23/02/2022 - Skip Trimming & Single-End Edition

A typcial command line looks like this:

~/AutDeNovo_exp_skiptrim.sh \                     ## The path to the shell script
...
Rev=/media/seq/rev.fq.gz \                        ## The full path to the raw read reverse FASTQ file (optional for single-end)
...
SkipTrimming=yes \                                ## Skip the trimming step and copy raw files to expected trimmed file names [optional; default=no]
SingleEnd=yes \                                   ## Process only forward reads, no reverse reads required [optional; default=no, auto-detected if Rev is empty]
...
if [ -z "$SkipTrimming" ]; then SkipTrimming="no"; fi
if [ -z "$SingleEnd" ]; then SingleEnd="no"; fi

## Test which data are available
if [[ !(-z "$fwd") && "$SingleEnd" == "yes" && -z "$ont" && -z "$pb" ]]; then
  data="ILL_SE"
elif [[ !(-z "$fwd") && "$SingleEnd" == "yes" && !(-z "$ont") && -z "$pb" ]]; then
  data="ILL_SE_ONT"
elif [[ !(-z "$fwd") && "$SingleEnd" == "yes" && !(-z "$pb") && -z "$ont" ]]; then
  data="ILL_SE_PB"
elif [[ !(-z "$fwd") && "$SingleEnd" == "yes" && !(-z "$pb") && !(-z "$ont") ]]; then
  data="ILL_SE_ONT_PB"
elif [[ !(-z "$fwd") && "$SingleEnd" == "no" && -z "$ont" && -z "$pb" ]]; then
  data="ILL"
elif [[ !(-z "$fwd") && "$SingleEnd" == "no" && !(-z "$ont") && -z "$pb" ]]; then
  data="ILL_ONT"
elif [[ !(-z "$fwd") && "$SingleEnd" == "no" && !(-z "$pb") && -z "$ont" ]]; then
  data="ILL_PB"
elif [[ !(-z "$fwd") && "$SingleEnd" == "no" && !(-z "$pb") && !(-z "$ont") ]]; then
  data="ILL_ONT_PB"
...
if [[ !(-z $fwd) ]]; then
  if [[ $PrintOnly == "no" ]]; then
    ...
    if [[ "$SingleEnd" == "no" ]]; then
      cp ${rev} ${out}/data/Illumina/${name}_2.fq.gz
    fi
  fi

  if [[ "$SingleEnd" == "yes" ]]; then
    printf "## Illumina single-end data copied\n# " |
      tee -a ${out}/shell/pipeline.sh
  else
    printf "## Illumina paired-end data copied\n# " |
      tee -a ${out}/shell/pipeline.sh
  fi
  date |
    tee -a ${out}/shell/pipeline.sh
...
if [[ !(-z $fwd) ]]; then
  ...
  if [[ "$SkipTrimming" == "yes" ]]; then
    
    printf "# Skipping trimming - copying raw files to expected trimmed file names\n# " |
      tee -a ${out}/shell/pipeline.sh
    date |
      tee -a ${out}/shell/pipeline.sh

    if [[ $PrintOnly == "no" ]]; then
      # Copy raw files to expected trimmed file names so downstream steps work
      cd ${out}/data/Illumina
      cp ${name}_1.fq.gz ${name}_1_val_1.fq.gz
      if [[ "$SingleEnd" == "no" ]]; then
        cp ${name}_2.fq.gz ${name}_2_val_2.fq.gz
      fi
      echo "## Raw files copied to trimmed file names for downstream compatibility"
    fi

    printf "## Skip trimming completed\n# " |
      tee -a ${out}/shell/pipeline.sh
    date |
      tee -a ${out}/shell/pipeline.sh
    ...
    if [[ "$SingleEnd" == "yes" ]]; then
      # Create a custom single-end trimming script inline
      printf "## Single-end trimming with ${Trimmer}\n# " |
        tee -a ${out}/shell/pipeline.sh
      
      # Generate single-end trimming script
      echo """
#!/usr/bin/env bash

## Single-end trimming for ${name}
#PBS -N trim_${Trimmer}_se_${name}
#PBS -o ${out}/log/trim_${Trimmer}_se_${name}_log.txt
#PBS -j oe
...
if [[ \"${Trimmer}\" == \"trimgalore\" ]]; then
  trim_galore \\
    ...
    --gzip \\
    ${name}_1.fq.gz
  # trim_galore outputs ${name}_1_trimmed.fq.gz, rename to expected format
  if [[ -f ${name}_1_trimmed.fq.gz ]]; then
    mv ${name}_1_trimmed.fq.gz ${name}_1_val_1.fq.gz
  fi
...
""" >${out}/shell/qsub_trim_se_${name}.sh

      if [[ $PrintOnly == "no" ]]; then
        if [[ $openpbs != "no" ]]; then
          qsub -W block=true ${out}/shell/qsub_trim_se_${name}.sh
        else
          bash ${out}/shell/qsub_trim_se_${name}.sh &>${out}/log/trim_${Trimmer}_se_${name}_log.txt
        fi
      fi

    else
      # Use existing paired-end trimming
